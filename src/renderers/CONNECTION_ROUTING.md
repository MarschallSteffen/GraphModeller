...
- **R3.5** Elbow mode (`'auto' | 'min' | 'max'`) restricts the **target** port axis. `'min'` restricts to horizontal sides (`e`/`w`) — connection enters target from the side. `'max'` restricts to vertical sides (`n`/`s`) — connection enters from top/bottom. This is axis-stable: the allowed set never changes as elements move, so the port cannot flip. If the element has no ports on the restricted axis (e.g. a queue on `'max'`), the restriction is silently dropped and auto-routing applies. Source port is always chosen freely by best score.
